...
export default async function executeCpp(code, input) {
  const timeoutSec = process.env.EXECUTION_TIMEOUT || "5";
  const command = `bash -c "g++ /code/Main.cpp -o /code/main 2> /code/compile_output.txt; if [ \\$? -ne 0 ]; then echo '___COMPILE_ERROR___'; cat /code/compile_output.txt; exit 111; fi; /usr/bin/time -f \\"time:%e\\\\nmemory:%M\\" -o /code/metrics.txt timeout ${timeoutSec}s /code/main < /code/input.txt"`;
